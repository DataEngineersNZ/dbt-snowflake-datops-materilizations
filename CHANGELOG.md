...
### Bug Fixes
* Fixed `enable_tasks` failing under dbt 2.0 / Fusion with `JinjaError: not a key type: ref not found for package: <pkg>, model: <name>`. The 1.0.7 change routed root/parent task relation resolution through `ref(package_name, name)`, but those task nodes are discovered by walking the graph and are not statically-declared dependencies of the `enable_tasks` operation — Fusion enforces stricter dependency-graph validation than dbt Core and rejects such dynamic `ref()` calls. `snowflake__task.sql` already documented and avoided this exact pattern for its own top-parent lookup; `enable_tasks` now uses the same fix — `api.Relation.create()` built from the current invocation's `target.database` plus the node's own `schema`/`alias` — instead of `ref()`.
...
* Fixed `resolve_relation_ref` assigning `found_node` with a plain `{% set %}` inside a `{% for %}` loop — Jinja for-loops have their own scope, so the assignment never escaped the loop and every lookup fell through to the "not found" compiler error. Switched to a `namespace()` object. Also added an explicit ambiguity check, matching `ref()`'s own precedence: a match in the current model's package always wins over matches in dependency packages, and only a genuine tie within the same precedence tier raises a compiler error instead of silently resolving to whichever node was visited last.
* Fixed `enable_tasks`, `snowflake__task`, and `resolve_relation_ref` all building the Snowflake identifier from a graph node's `name` (its logical/file name) instead of its `alias` (the name it was actually created under). Any task, network rule, or secret using a custom dbt `alias` would have been suspended/resumed/referenced under a nonexistent object name.
* Fixed `enable_tasks`, `snowflake__task`'s top-parent lookup, and `resolve_relation_ref` all hard-coding `target.database` (or the current model's own `database`) for graph nodes that are not statically-declared dependencies. Added a `resolve_node_database` helper: it uses the node's own `database` config when the node explicitly overrides it (detected via `node.unrendered_config`, since `node.database` is always populated whether overridden or not), and otherwise falls back to the current invocation's `target.database` to preserve the 1.0.7 fix for stale cached databases on reused manifests.
* Fixed `enable_tasks`, `snowflake__task`, and `resolve_relation_ref` all building relations with `api.Relation.create()` without a `quote_policy`, silently falling back to the adapter's default quoting instead of the referenced node's own resolved quote policy (`node.config.quoting`). A node created with quoted identifier/database/schema names would otherwise be looked up under the wrong quoting and could resolve to a different or nonexistent object. Consolidated all three call sites into a single `resolve_node_relation` helper that combines `resolve_node_database`, `node.alias`, and `node.config.quoting`.
* Fixed `resolve_relation_ref` raising an ambiguity error for a versioned model (multiple manifest nodes sharing a name via dbt's model-versioning feature) even when one of them is resolvable as the "latest" version, which is what an unversioned `ref('name')` would silently bind to. Only genuinely ambiguous nodes (no resolvable latest version) now raise.
* Closed a coverage gap where `resolve_relation_ref` was never actually executed by either CI job (the only model that used it, `test_external_access_integration`, is excluded from `dbt run` since it needs `ACCOUNTADMIN`). Added `assert_resolve_relation_ref`, a `run-operation` that calls the resolver against the already-built `test_network_rule`/`test_secret` models and asserts on the result — exercising the graph lookup, alias, and database resolution logic at runtime without ever issuing `CREATE EXTERNAL ACCESS INTEGRATION`.
...
* Fixed the Fusion job's standalone `dbt parse`/`dbt compile` check failing with `Schema '...' does not exist or not authorized` — it ran before the on-run-start hooks created the target schema, and `dbt compile` (unlike dbt Core) eagerly executes `{% call statement(...) fetch_result=True %}` blocks in test files during compilation. Scoped the compile step to `--exclude resource_type:test`.
* Hardened both `--exclude tag:a tag:b`-style model-run steps into two separate `--exclude` flags, removing reliance on undocumented multi-value single-flag parsing that may not be portable between the dbt Core and Fusion CLIs.
