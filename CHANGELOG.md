...
* Fixed `enable_tasks`, `snowflake__task`, and `resolve_relation_ref` (`external_access_integration`) all building the Snowflake identifier from a graph node's `name` (its logical/file name) instead of its `alias` (the name it was actually created under). Any task, network rule, or secret using a custom dbt `alias` would have been suspended/resumed/referenced under a nonexistent object name.
* Fixed `resolve_relation_ref` assigning `found_node` with a plain `{% set %}` inside a `{% for %}` loop — Jinja for-loops have their own scope, so the assignment never escaped the loop and every lookup fell through to the "not found" compiler error. Switched to a `namespace()` object. Also added an explicit ambiguity check so a name matching more than one node (e.g. across packages) now raises a clear compiler error instead of silently resolving to whichever node was visited last.
