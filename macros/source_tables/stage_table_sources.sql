    ...
    {% if execute %}
            ...
            {% if target.profile_name in enabled_profiles %}
                {% if target.name in enabled_targets %}
                    ...
                    {% do stage_table_sources_plans(sources_to_stage_auto_maintained, true, 'internal', true) %}
                    {% do stage_table_sources_plans(sources_to_stage_auto_maintained, false, 'internal', true) %}
                    {% do log('===> ' ~ sources_to_stage_no_maintenance|length ~  ' Tables only to be created by dbt <===', info = true) -%}
                    {% do stage_table_sources_plans(sources_to_stage_no_maintenance, true, 'internal', false) %}
                    {% do log('===> ' ~ externals_tables_to_stage_auto_maintained|length ~  ' Tables to be maintained by dbt (with external source) <===', info = true) -%}
                    {% do stage_table_sources_plans(externals_tables_to_stage_auto_maintained, true, 'external', true) %}
                    {% do stage_table_sources_plans(externals_tables_to_stage_auto_maintained, false, 'external', true) %}
                    {% do log('===> ' ~ externals_tables_to_stage_no_maintenance|length ~  ' Tables only to be created by dbt  (with external source) <===', info = true) -%}
                    {% do stage_table_sources_plans(externals_tables_to_stage_no_maintenance, true, 'external', false) %}
                {% else %}
...
{% macro stage_table_sources_plans(sources_to_stage, isFirstRun, table_type, auto_maintained) %}
    {% for node in sources_to_stage %}
        ...
        {% set run_queue = get_source_build_plan(node, isFirstRun, table_type, auto_maintained) %}
        {% if run_queue == [] %}
