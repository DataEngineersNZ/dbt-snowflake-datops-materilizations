...
{% macro stage_table_sources() %}
    {% if flags.WHICH == 'run' %}
        ...
        {% do dbt_dataengineers_utils_materilizations.stage_table_sources_plans(sources_to_stage, true, false) %}
        {% do dbt_dataengineers_utils_materilizations.stage_table_sources_plans(sources_to_stage, false, false) %}
        {% do dbt_dataengineers_utils_materilizations.stage_table_sources_plans(streams_to_stage, false, true) %}
    {% endif %}
{% endmacro %}
...
{% macro stage_table_sources_plans(sources_to_stage, isFirstRun, isStream) %}
    {% for node in sources_to_stage %}
        ...
        {% else %}
            ...
            {% endif %}
        {% endif %}
        {% set run_queue = dbt_dataengineers_utils_materilizations.get_source_build_plan(node, isFirstRun, isStream) %}
        {% if isStream %}
