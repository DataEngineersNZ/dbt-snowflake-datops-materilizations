...
{% macro stage_table_sources(enabled_targets) %}
    {% if target.name in enabled_targets %}
        {% if flags.WHICH == 'run' %}
            ...
            {% do dbt_dataengineers_materializations.stage_table_sources_plans(sources_to_stage, true, 'internal') %}
            {% do dbt_dataengineers_materializations.stage_table_sources_plans(sources_to_stage, false, 'internal') %}
            {% do dbt_dataengineers_materializations.stage_table_sources_plans(externals_tables_to_stage, true, 'external') %}
            {% do dbt_dataengineers_materializations.stage_table_sources_plans(externals_tables_to_stage, false, 'external') %}
        {% endif %}
    {% else %}
