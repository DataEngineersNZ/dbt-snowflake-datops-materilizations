{% macro snowflake__get_source_build_plan(source_node, is_first_run, auto_maintained) %}
    ...
    {% if is_first_run %}
        ...
        {%- set stream_name = snowflake_get_stream_name(identifier) -%}
        ...
        {% if current_relation_exists_as_view %}
            {% do build_plan.append(snowflake_drop_view(current_relation)) %}
        ...
        {% if current_relation_exists_as_table and auto_maintained %}
            {% if source_node.external.retain_previous_version_flg %}
                ...
                {%- set backup_table_suffix = config_meta_get('backup_table_suffix', '_DBT_BACKUP_') -%}
                ...
                {% do build_plan.append(snowflake_clone_table_relation_if_exists(current_relation, backup_relation)) %}
            {% endif %}
            {% do build_plan.append(snowflake_clone_table_relation_if_exists(current_relation, migration_relation)) %}
        {% endif %}
        {# drop and re-create tables as necesary #}
        {% if create_or_replace %}
            {% if current_relation_exists_as_table %}
                {% do build_plan.append(snowflake_drop_stream_statement(stream_relation)) %}
                {% do build_plan.append(snowflake_drop_table(current_relation)) %}
            {% else %}
                {% do build_plan.append(snowflake_create_schema(target_relation)) %}
            {% endif %}
            {% do build_plan.append(snowflake_create_or_replace_table(target_relation, source_node)) %}
        {% elif auto_maintained %}
            {% do build_plan.append(snowflake_create_or_replace_table(comparison_relation, source_node)) %}
        {% endif %}
    {% elif auto_maintained %}
        {% if current_relation is not none and comparison_relation is not none %}
            ...
            {% if not full_refresh_mode %}
                ...
                {% if new_cols|length > 0 -%}
                    {% for col in new_cols %}
                        {% do build_plan.append(snowflake_alter_table_add_column(current_relation, col.name, col.data_type)) %}
                    {% endfor %}
                {%- endif %}
                ...
                {% if dropped_cols|length > 0 -%}
                    {% for col in dropped_cols %}
                        {% do build_plan.append(snowflake_alter_table_drop_column(current_relation, col.name)) %}
                    {% endfor %}
                {%- endif %}
                ...
                {% for new_col in new_cols_sizing %}
                    {% for old_col in old_cols_sizing %}
                        {% if new_col.name == old_col.name and new_col.data_type != old_col.data_type  %}
                            {% do build_plan.append(snowflake_alter_table_alter_column(current_relation, old_col.name, new_col.data_type)) %}
                        {% endif %}
                    {% endfor %}
                {% endfor %}
            ...
            {% else %}
                {% if source_node.external.migrate_data_over_flg %}
                    {% do build_plan.append(snowflake_migrate_data(migration_relation, target_relation, source_node)) %}
                {% endif %}
            {% endif %}
        {% endif %}
        {# Tidy up - drop comparison and migration tables #}
        {% do build_plan.append(snowflake_drop_table(migration_relation)) %}
        {% do build_plan.append(snowflake_drop_table(comparison_relation)) %}
    {% endif %}
