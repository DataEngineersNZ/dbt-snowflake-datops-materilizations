        ...
        {% if source_node.external.get('snowpipe', none) is not none %}
            {% if create_or_replace %}
                {% set build_plan = build_plan + [
                    ...
                    snowflake_create_empty_table(target_relation, source_node),
                    snowflake_get_copy_sql(target_relation, source_node, explicit_transaction=true),
                    snowflake_create_snowpipe(target_relation, source_node)
                ] %}
            {% elif auto_maintained %}
                {% set build_plan = build_plan + [
                    snowflake_create_or_replace_table(comparison_relation, source_node),
                    snowflake_clone_table_relation_if_exists(current_relation, migration_relation)
                    ] %}
            {% endif %}
        {% else %}
            {% if create_or_replace %}
                {% set build_plan = build_plan + [ snowflake_create_external_table(target_relation, source_node) ] %}
            {% else %}
                {% set build_plan = build_plan + snowflake_refresh_external_table(target_relation,source_node) %}
            {% endif %}
        {% endif %}
    {% elif auto_maintained %}
        {% if current_relation is not none and migration_relation is not none %}
            ...
            {% if new_cols|length > 0 -%}
                {% if source_node.external.get('snowpipe', none) is not none %}
                    {% set build_plan = build_plan + [
                        snowflake_drop_pipe(target_relation),
                        snowflake_drop_table(current_relation),
                        snowflake_create_empty_table(target_relation, source_node),
                        snowflake_migrate_data(migration_relation, target_relation, source_node),
                        snowflake_create_snowpipe(target_relation, source_node),
                        snowflake_drop_table(comparison_relation),
                        snowflake_drop_table(migration_relation)
                        ] %}
                    {% endif %}
            {% else %}
                {% set build_plan = build_plan + [
