{% macro snowflake_get_task_parent_node(node) -%}
  ...
  {% if node %}
    {% if node.config.materialized == "task" %}
      {% if node.depends_on.nodes|count > 0 %}
        {% for index in range(0, node.depends_on.nodes|count) %}
          {% set model_detail = dbt_dataengineers_materializations.get_task_node_by_id(node.depends_on.nodes[index]) %}
